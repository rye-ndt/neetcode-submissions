func exist(board [][]byte, word string) bool {
	...
	backtrack = func(path [][2]int) bool {
		...
		if str != word[:len(str)] || len(path) == len(word) { return false }
		...
		for _, o := range options {
			...
			if !valid(newX, len(board[0])) || !valid(newY, len(board)) || used(path, newX, newY) {
				continue
			}

			if backtrack(append(path, [2]int{newX, newY})) {
				return true
			}
		}
